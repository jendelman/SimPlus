...
#' Update kinship file
#'
#' Kinship is computed for selection candidates in \code{pop}. For \code{K.method}="A", argument \code{ped.file} is the three-column pedigree file. For \code{K.method}="G.VR", argument \code{p.ref} is the vector of reference allele frequencies. To reduce computing time for \code{K.method}="G.IBD", only \code{ibd.loci} are sampled per chromosome. 
#' 
#' @param pop variable of AlphaSimR Pop-class
#' @param SP simulation parameters for AlphaSimR
#' @param K.method kinship method: A, G.VR, G.IBD
#' @param K.file kinship CSV filename
#' @param ped.file pedigree CSV filename
#' @param p.ref reference pop allele frequencies for "VR" method
...
update_K <- function(pop, SP, K.method, K.file, ped.file=NULL, 
  ...
  stopifnot(K.method %in% c("A","G.VR","G.IBD"))
